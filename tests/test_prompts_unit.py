...
def test_system_prompt_template_has_placeholders():
    for key in ("db_schemas", "contractors_str", "exact_work_types_str",
                "work_types_str", "objects_str", "cities_str"):
...
def test_build_messages_roles():
    ...
    with (
        ...
        patch(
            "src.sql_layer.prompts.get_schema_from_db", return_value=FAKE_SCHEMAS
        ),
        patch(
            "src.sql_layer.prompts.build_prompt_values", return_value=FAKE_VALUES
        ),
    ):
